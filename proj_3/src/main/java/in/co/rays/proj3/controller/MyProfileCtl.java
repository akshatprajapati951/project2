...
public class MyProfileCtl extends BaseCtl {
	...
	protected boolean validate(HttpServletRequest request) {
		...
		if (DataValidator.isNull(request.getParameter("firstName"))) {
			...
			pass = false;
		} else if (request.getParameter("firstName").contains(" ")) {
			request.setAttribute("firstName", PropertyReader.getValue("Space not allowed"));
			pass = false;
		}
		...
		if (request.getParameter("gender").equalsIgnoreCase("N/A")) {
		...
		if (DataValidator.isNull(request.getParameter("dob"))) {
			...
			pass = false;
		} else if (DataValidator.getAge(request.getParameter("dob")) < 18) {
