...
public class ChangePasswordCtl extends BaseCtl {
	...
	protected boolean validate(HttpServletRequest request) {
		...
		if (DataValidator.isNull(request.getParameter("newPassword"))) {
			...
			pass = false;
		} else if (!(DataUtility.getStringData(request.getParameter("newPassword")).length() > 8)) {
			request.setAttribute("newPassword", PropertyReader.getValue("error.bigpass", "Password"));
			pass = false;
		} else if (!(DataUtility.getStringData(request.getParameter("newPassword")).length() < 12)) {
			request.setAttribute("newPassword", PropertyReader.getValue("error.smallpass", "Password"));
			pass = false;
			...
			request.setAttribute("newPassword",PropertyReader.getValue("error.require1", "old and new Password"));
			pass=false;
		...
		if (DataValidator.isNull(request.getParameter("confirmPassword"))) {
			...
			request.setAttribute("confirmPassword",PropertyReader.getValue("error.require2", "confirm and new Password"));
			pass = false;
		}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (OP_UPDATE.equalsIgnoreCase(op)) {
			try {
				...
				ServletUtility.setErrorMessage("Old password does not exits!", request);
			}
